...
// Save key -> body: { "key": "images/xyz.jpg" }
imagesRouter.post("/save-key", async (req, res) => {
  try {
    ...
    const q = `INSERT INTO images (r2_key) VALUES ($1)
               ...
               RETURNING id, r2_key, created_at;`;
    const { rows } = await pool.query(q, [key]);
    return res.json({ ok: true, image: rows[0] });
  } catch (err) {
...
imagesRouter.get("/:id", async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      "SELECT id, r2_key AS key, created_at FROM images WHERE id=$1",
      [id]
    );
    if (!rows.length) {
